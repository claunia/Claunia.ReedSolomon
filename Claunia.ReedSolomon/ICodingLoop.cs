...
namespace Claunia.ReedSolomon
{
    public interface ICodingLoop
    {
        ...
        ///     and checks that the data is those shards matches what's expected.
        /// </summary>
        /// <param name="matrixRows">The rows from the matrix to use.</param>
        /// <param name="inputs">
        ///     An array of byte arrays, each of which is one input shard. The inputs array may have extra buffers
        ///     after the ones that are used.  They will be ignored.  The number of inputs used is determined by the length of the
        ///     each matrix row.
        /// </param>
        /// <param name="inputCount">The number of input byte arrays.</param>
        /// <param name="outputs">
        ///     Byte arrays where the computed shards are stored.  The outputs array may also have extra, unused,
        ///     elements at the end.  The number of outputs computed, and the number of matrix rows used, is determined by
        ///     outputCount.
        /// </param>
        ...
        void CodeSomeShards(byte[][] matrixRows, byte[][] inputs, int inputCount, byte[][] outputs, int outputCount,
                            int offset, int byteCount);

        /// <summary>
        ///     Multiplies a subset of rows from a coding matrix by a full set of input shards to produce some output shards,
        ///     and checks that the the data is those shards matches what's expected.
        /// </summary>
        /// <param name="matrixRows">The rows from the matrix to use.</param>
        /// <param name="inputs">
        ///     An array of byte arrays, each of which is one input shard. The inputs array may have extra buffers
        ...
        /// <returns></returns>
        bool CheckSomeShards(byte[][] matrixRows, byte[][] inputs, int inputCount, byte[][] toCheck, int checkCount,
